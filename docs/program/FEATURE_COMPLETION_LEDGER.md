...
| Rental marketplace | Core rental full lifecycle and dashboard refresh path | Implemented locally | Real PostgreSQL persistence, live RLS/RBAC, Supabase Auth identity, storage-backed contracts, payment capture, escrow, staging validation, and production provider evidence missing | Keep the customer/supplier lifecycle and dashboard list path behind `rental_core_backend_path` until A4 and provider evidence pass | ACCEL-P1-007 |
| Rental marketplace | Core rental production-readiness bridge | Implemented locally | Bridge contracts and executable SQL are prepared only; PostgreSQL execution, live RLS/RBAC, Supabase Auth, Supabase Storage, payment sandbox, staging journey, and production provider evidence remain missing | Use migration 008, bridge contracts, storage manifest, payment contract, and staging test plan once A4 infrastructure and executable database/provider paths exist | ACCEL-P1-008 |
...
| Core platform | Migrations 001-008 | Implemented locally | Live reset/migration evidence missing | Run local reset once tooling/environment permits | A4-03 |
